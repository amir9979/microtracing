...
public class Span{
	...
	public void printTagLog(String tagName){
		String value = getTag(tagName);
	...
	public void stop(){
		...
		if (tracer.getThreadRootSpan()==this){
			tracer.clear(); //clear tracer
		}else if (tracer.getCurrentSpan()==this) {
	...
	private void printEvent(SpanEvent event){
		String log = event.toString();
		if (SPAN_START.equals(event.getEvent())) {
			log = this.toString() + " " +log;
		}
		log(log);
	}
